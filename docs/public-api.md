...
- GET `/hero` — вернуть активный геро‑блок или `null`, если активного нет.

Описание: Витрина может запросить единичный управляемый блок «герой» для главной. В админке может быть несколько черновиков, но активен всегда максимум один. Если активного нет — возвращается `null`, фронт рисует дефолтный герой.
...
Ответ 200 (когда героя нет или он выключен):
...
- Поля `titleI18n`/`subtitleI18n`/`cta.labelI18n` — опциональные, фронт сам выбирает язык (uk обязателен в админке, en опционален, фолбеков на сервере нет).
